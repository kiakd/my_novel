#!/usr/bin/env python3
# Cantarella POV (LoRA: pov_lookingdown_il3) — MLD=missionary / MPLD=mating press / DSLD=doggy
import json, urllib.request, time, sys

SERVER = "127.0.0.1:8188"
MODE = sys.argv[1] if len(sys.argv) > 1 else "MLD"
SEED = int(sys.argv[2]) if len(sys.argv) > 2 else 33144250

POSE = {
    "MLD":  "MLD, on back, straddling",
    "MPLD": "MPLD, legs up, mating press",
    "DSLD": "DSLD, from behind, butt, all fours",
}[MODE]
...
POS = ("masterpiece, best quality, amazing quality, absurdres, usnr, "
       ...
       f"1girl, 1boy, pov, lying down on ground, {POSE}, pinning, pov arms, "
...
STACK = [("stabilizer.safetensors", 0.7, 0.7),
         ("usnr_style.safetensors", 0.5, 0.5),
         ("pov_lookingdown_il3.safetensors", 0.85, 0.85),
...
wf["9"] = {"class_type": "SaveImage", "inputs": {"filename_prefix": f"cantarella_pov_{MODE}", "images": ["8", 0]}}
...
print(f"queued {MODE} seed={SEED}", flush=True)
t0 = time.time()
