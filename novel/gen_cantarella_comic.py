#!/usr/bin/env python3
# Cantarella comic-page style (LoRA: fcomic_hardcore_il_v2) — สร้างหน้าคอมมิคหลายช่องในรูปเดียว
import json, urllib.request, time, sys

SERVER = "127.0.0.1:8188"
SEED = int(sys.argv[1]) if len(sys.argv) > 1 else 778451023

POS = ("fcomichardcore, (comic:1.25), multi view, frame, speech bubble, "
       "comic expression, focus line, sound effect, "
       "masterpiece, best quality, amazing quality, absurdres, "
       "nsfw, rating:explicit, WW Cantarella, mature female, adult, "
       "long silver blue hair, large breasts, 1girl, solo focus, 1boy, hetero, sex, "
       "nude, blush, sweat, multiple panels, sequential")
NEG = ("bad quality, worst quality, worst detail, lowres, bad anatomy, bad hands, "
       "missing fingers, extra digits, fewer digits, extra limbs, fused fingers, "
       "jpeg artifacts, signature, watermark, username, blurry, child, loli, flat chest, young")

STACK = [("stabilizer.safetensors", 0.6, 0.6),
         ("fcomic_hardcore_il_v2.safetensors", 1.1, 1.1),
...
wf["3"] = {"class_type": "KSampler",
           "inputs": {"seed": SEED, "steps": 30, "cfg": 5.0, "sampler_name": "dpmpp_2m",
                      "scheduler": "karras", "denoise": 1.0, "model": m,
                      "positive": ["6", 0], "negative": ["7", 0], "latent_image": ["5", 0]}}
wf["8"] = {"class_type": "VAEDecode", "inputs": {"samples": ["3", 0], "vae": ["4", 2]}}
wf["9"] = {"class_type": "SaveImage", "inputs": {"filename_prefix": "cantarella_comic", "images": ["8", 0]}}
...
print(f"queued comic seed={SEED}", flush=True)
t0 = time.time()
while time.time() - t0 < 600:
    ...
    if pid in h:
        ...
        if h[pid].get("status", {}).get("status_str") == "error":
            print("ERROR", json.dumps(h[pid]["status"])[:1500]); break
