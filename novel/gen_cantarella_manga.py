#!/usr/bin/env python3
# Cantarella manga-style sequence (5 ช่อง) — ใช้ pose LoRA ที่โหลดไว้ ไล่ซีนต่อเนื่อง
# คงคาแรกเตอร์ด้วย cantarella LoRA + สไตล์ usnr + stabilizer ทุกช่อง
import json, urllib.request, time

SERVER = "127.0.0.1:8188"
Q = "masterpiece, best quality, amazing quality, absurdres, usnr, "
CH = "WW Cantarella, mature female, adult, beautiful detailed face, long silver blue hair, "
...
# common LoRA ที่ใส่ทุกช่อง: stabilizer + usnr style + cantarella character
def base_stack(extra=None):
    s = [("stabilizer.safetensors", 0.7, 0.7),
         ("usnr_style.safetensors", 0.5, 0.5),
         ("cantarella_main.safetensors", 0.8, 0.8)]
    if extra: s.insert(2, extra)   # pose lora ก่อน character
    return s

PANELS = [
    ("cantarella_seq_1_portrait", 101,  base_stack(),
     Q + CH + "1girl, solo, WW Cantarella Main Outfit, elegant dress, standing, "
     "looking at viewer, gentle smile, ornate hall, soft lighting, sfw"),
    ("cantarella_seq_2_nude", 102, base_stack(),
     Q + CH + "nsfw, rating:explicit, 1girl, solo, nude, large breasts, "
     "seductive pose, on bed, blush, parted lips, bedroom, soft warm lighting"),
    ("cantarella_seq_3_doggy", 103, base_stack(("squatting_doggystyle.safetensors", 0.85, 0.85)),
     Q + CH + "nsfw, rating:explicit, 1girl, 1boy, hetero, nude, doggystyle, sex from behind, "
     "all fours, ssfb, ass, arched back, looking back, blush, sweat, vaginal, bedroom"),
    ("cantarella_seq_4_matingpress", 104, base_stack(("mating_press.safetensors", 0.85, 0.85)),
     Q + CH + "nsfw, rating:explicit, 1girl, 1boy, hetero, nude, missionary, mating press, "
     "deep penetration, legs up, blush, sweat, looking at viewer, bedroom"),
    ("cantarella_seq_5_gangbang", 105, base_stack(("oral_gangbang.safetensors", 0.9, 0.9)),
     Q + CH + "nsfw, rating:explicit, 1girl, multiple boys, nude, large breasts, gangbang, "
     "surrounded by penises, fellatio, oral, cum, blush, kneeling, indoor"),
]
