...
export function ChatProvider({ children }: { children: ReactNode }) {
  ...
  const savedMeta = useRef<{ chars: ChatState['chars']; items: ChatState['items'] } | null>(null);
  ...
  const loadAll = useCallback(async (): Promise<boolean> => {
    ...
    savedMeta.current = { chars: next.chars, items: next.items };
  ...
  const doSave = useCallback(async () => {
    ...
    try {
      ...
      if (!savedMeta.current || savedMeta.current.chars !== cur.chars || savedMeta.current.items !== cur.items) {
        const res = await putChatState({ chars: cur.chars, items: cur.items }, metaRev.current);
        if (res.ok) {
          metaRev.current = res.rev ?? metaRev.current + 1;
          savedMeta.current = { chars: cur.chars, items: cur.items };
